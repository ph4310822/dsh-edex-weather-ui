...
/**
 * The center chrome's title strip: the workspace frame (the reshaped layout
 * frame) starts this far below the center region's top edge, so the 1px
 * border + 19px WEATHER WORKSPACE strip frame the workspace without ever
 * covering it (1px border + 19px strip = 20px inset).
 */
...
/**
 * The center region slot: wraps the ORIGINAL UI area in the same widget
 * chrome as the shell bars. The heading is rendered by the shell itself
 * (`.centerTitle`, the WEATHER WORKSPACE strip) — the WidgetSection stays
 * transparent below it so the original UI shows through untouched.
 */
...
export function EdexShell({
  ...
  useLayoutEffect(() => {
    ...
    // The center chrome's title strip occupies the top 20px (1px border +
    // 19px strip); the original UI starts below it so the chrome frames the
    // workspace without covering it (CENTER_TITLE_INSET).
  ...
  return (
    ...
    >
      ...
      {/* The ORIGINAL UI wrapped in the standard widget chrome: the WEATHER
          WORKSPACE title strip + 1px chrome border frame it like one more
          floating map-app panel (Windy's panel language); the WidgetSection
          below the strip stays transparent so the UI is never covered. */}
      <section className={css.centerWidget}>
        <div className={css.centerTitle} data-testid="edex-center-title">DSH WORKSPACE</div>
        <section className={css.centerFrame}>
